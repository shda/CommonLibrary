...
//         fixes it. Under the hood the state is still always valid, and serialized correctly regardless.
    ...
    {    
        ...
        private SceneAsset sceneAsset {
            get { return GuidToSceneAsset(guid); }
            set { guid = SceneAssetToGuid(value); }
        }
        ...
        public static SceneAsset GuidToSceneAsset(string guid) {
        ...
        public static string SceneAssetToGuid(SceneAsset value) {
    ...
    public class SceneReferencePropertyDrawer : PropertyDrawer
    {
        ...
        const string sceneGuidPropertyString = "guid";
        ...
        {
            var sceneGuidProperty = GetSceneGuidProperty(property);
            ...
            var sceneAsset = SceneReference.GuidToSceneAsset(sceneGuidProperty.stringValue);
            var selectedObject = EditorGUI.ObjectField(position, label, sceneAsset, typeof(SceneAsset), false) as SceneAsset;
            BuildUtils.BuildScene buildScene = BuildUtils.GetBuildScene(selectedObject);

            if (EditorGUI.EndChangeCheck()) {
                sceneGuidProperty.stringValue = SceneReference.SceneAssetToGuid(selectedObject);
        ...
        {
            int lines = 2;
            SerializedProperty sceneGuidProperty = GetSceneGuidProperty(property);
            var sceneAsset = SceneReference.GuidToSceneAsset(sceneGuidProperty.stringValue);
            if (sceneAsset == null)
                lines = 1;
        ...
        static SerializedProperty GetSceneGuidProperty(SerializedProperty property)
        {
            return property.FindPropertyRelative(sceneGuidPropertyString);
        }
